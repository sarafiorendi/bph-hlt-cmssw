  ...
  beamSpotToken_(consumes<reco::BeamSpot>(beamSpotTag_))
{
  ...
  //for internal tests
  produces<RecoChargedCandidateDoubleMap>("d0firstTrk" );
  produces<RecoChargedCandidateDoubleMap>("LSigma"     );
  produces<RecoChargedCandidateDoubleMap>("Cosine"     );
  produces<RecoChargedCandidateDoubleMap>("VertexCL"   );
}
...
void HLTmumutkVtxProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)  
{
  ...
  //only for internal use: to delete
  ...
  for (mucand1=mucands->begin(); mucand1!=mucands->end(); ++mucand1) {
    ...
    for (; mucand2!=mucands->end(); mucand2++) {
      ...
      //following line for internal test
...
//         if (d0sig < minD0Significance_) continue;
        ...
        // only for internal tests, evaluate L/sigma 
        double vtxProb = 0.0;
